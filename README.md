...
- 登录用户通过统一的 `/submit` 页面提交作品，选择一个主分类和 1-6 个受控标签，明确标记原创、转载或改编，可多选实际验证过的 AI 工具，并可标记 NSFW；投稿默认进入审核，审核结果进入站内通知，待审或驳回作品可修改后重新提交
...
- 来源平台与生成工具分开管理；来源平台的名称、Logo、品牌色、官网、排序与启停均来自 Supabase，Logo 可使用 LobeHub/Simple Icons 或官方素材初始化后上传到 R2，前端不依赖外部图标 CDN
- Cloudflare R2 保存生产图片；上传登记与定时补偿任务自动清理未完成投稿或内容删除产生的孤儿对象；独立的私有 R2 桶长期归档压缩后的作品互动明细；Vercel 部署并通过 Web Analytics 与 Speed Insights 观察匿名访问和真实用户性能
- Supabase 是作品内容的唯一数据源；数据库为空时首页不展示作品
- 全站采用统一响应式页面骨架：页头、页脚、首页和后台可扩展到 128rem，详情、专栏、账户与投稿使用 90rem 内容容器；后台左侧导航和详情信息栏在桌面端吸顶，手机端自动回流为单列
- 首页保留手机单列、常规桌面双列、宽屏三列的克制瀑布流；仅在超宽屏利用原内容区之外的空间展示左侧发现栏，右侧只在确有编辑精选时出现，不使用说明卡填补空白；较窄视口保留顶部横向筛选；独立专栏页按主题组织作品
- 首页采用服务端瀑布流数字分页：首张关键封面优先，其余图片原生懒加载；公开详情、单分类与单标签落地页进入 sitemap，组合筛选避免重复收录
